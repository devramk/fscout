...
pub fn get_start_xref(file: &mut File) -> std::io::Result<HashMap<u64, u64>> {
    ...
    let mut xref_start_idx: u64 = 0;
    if startxref_found && line_str.len() > 0 {
        xref_start_idx = line_str.trim().parse::<u64>().unwrap_or_else(|_| 0);
    }
    if xref_start_idx > 0 {
        file_bytes.get_mut().rewind()?;
        file_bytes.seek(SeekFrom::Start(xref_start_idx + 5))?;
        ...
        for xref_bytes in file_bytes.by_ref().bytes() {
            ...
            if curr_str == "\n" {
                ...
                if line_parts.len() <= 2 {
                    // first_object = line_parts[0].parse::<u64>().unwrap_or_else(|_| 0);
